<#
    ...
    Accounts created within 7 days of the script running will not be disabled or deleted, if no sign-in activity has been found on the account.
    Using the Undo parameter will allow the script to reverse the last set of changes provided by the Path parameter.
    When run without any parameters the script gathers a list of admin accounts it would have performed actions on.
    ...
    .EXAMPLE
    Exports list of admin accounts it would have taken action on.

    PS> .\AdminAccountLifecycle.ps1

    .EXAMPLE
    Disables accounts that haven't signed in for the last 365 days.

    PS> .\AdminAccountLifecycle.ps1 -Disable

    .EXAMPLE
    Deletes all accounts that have been disabled by the script over 60 days ago.

    PS> .\AdminAccountLifecycle.ps1 -Delete

    .EXAMPLE
    Disables accounts that haven't signed in for the last 365 days and deletes all accounts that have been disabled by the script over 60 days ago.

    PS> .\AdminAccountLifecycle.ps1 -Disable -Delete

    .EXAMPLE
    Undo all changes captured in the action CSV file provided by the Path parameter.

    PS> .\AdminAccountLifecycle.ps1 -Undo -Path .\AdminAccountLifecycleActions-202502211446.csv
#>
